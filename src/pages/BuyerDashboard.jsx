...
const BuyerDashboard = () => {
  ...
  const filteredProduce = availableProduce.filter(
    (item) => selectedCategory === 'all' || item.category === selectedCategory
  );

  const handleBooking = (produceId) => {
              ...
              <div>
                ...
                <div className="space-y-2">
                  {['today', 'tomorrow', 'this-week'].map((date) => (
                    ...
                    >
                      {date.replace('-', ' ').charAt(0).toUpperCase() +
                        date.replace('-', ' ').slice(1)}
                    </button>
                  ))}
                </div>
              </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${((produce.total - produce.available) / produce.total) * 100}%`
                          }}
                        ></div>
                      </div>
                    </div>
